...
t1c_all_v0 <- cbind(t1c_gsib_us,t1c_dsib_us,t1c_ca)%>%
...
t1c_bankwise_mean <- t1c_all_v0%>%
  group_by(bank)%>%
  summarise(bank_mean_B=mean(t1c_B, na.rm=T))%>%
  arrange(desc(bank_mean_B))%>%
  mutate(Country = if_else(bank %in% c('ry', 'td', 'bmo', 'cm', 'na', 'nbs'), 
                           "Canada",
                           "The US"))
t1c_bankwise_mean_plt <- t1c_bankwise_mean %>%
  filter(bank != "mufg") %>%  # Filter out "mufg"
  ggplot(aes(x = reorder(bank, -bank_mean_B), 
             y = bank_mean_B,
             fill = Country)) +  # Map fill to the new column
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("Canada" = "steelblue", "The US" = "gray")) +  # Custom colors
  labs(
    title = "Mean T1C by Bank (Excluding MUFG)",
    x = "Bank",
    y = "Mean T1C (in Billions)"
  ) +
  theme_minimal() +
...
t1c_trend_by_bank_plt <- ggplot(t1c_all_v0 %>% filter(!is.na(t1c) & bank != "mufg" & Date < "2019-01-01"),
       aes(Date, t1c_B, color = as.factor(bank), group = as.factor(bank))) +
  geom_line() +
  theme_minimal() +
  labs(
    title = "Tier 1 Capital Over Time by Bank",
    subtitle = "2006 - 2019, excluding MUFG",
    x = "Date",
